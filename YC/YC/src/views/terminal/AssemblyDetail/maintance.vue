                    ...
                    <Col span="8">
                      <Form :label-width="120" :style="{marginLeft:'-44px',marginTop:'30px'}">
                          <FormItem label="报表模板:" :style="{width:'200px'}" id="myform">
                             <Button type="ghost" icon="ios-cloud-download-outline" @click="download">点击下载</Button>
                         </FormItem>
                      </Form>
                    </Col>
                        ...
                        <Form :label-width="120" :style="{marginLeft:'-146px',marginTop:'30px'}">
                            <FormItem label="表格导入:" :style="{width:'200px'}" id="myform"  v-if="file == null">
                              <Upload
                                 ...
                                 action="`${this.URL_PREFIX}/supplies/maintain/upload`">
...
export default {
     ...
     methods:{
       ...
       */
       ...
       getDataList() {
         this.$axios.get(`${this.URL_PREFIX}/supplies/maintain?page=1&size=${this.pageSize}`).then( res => {
             if(res.data.code == '0'){
       ...
       Search(){
         this.loading = true
         this.$axios.get(`${this.URL_PREFIX}/supplies/maintain?page=1&size=${this.pageSize}&starttime=${this.starttime}&endtime=${this.endtime}&work=${this.station}&type=${this.mode}&model=${this.type}&keyword=${this.keyword}`)
               .then(res => {
                 if(res.data.code == '0'){
       ...
       Expor(){
         this.loading1 = true
         this.$axios.get(`${this.URL_PREFIX}/supplies/maintain/export?starttime=${this.starttime}&endtime=${this.endtime}&work=${this.station}&type=${this.mode}&model=${this.type}&keyword=${this.keyword}`).then( res => {
             if(res.data.code == '0'){
               setTimeout(() => {
                 this.loading1 = false
       ...
       upload() {
         ...
         this.$axios.post(`${this.URL_PREFIX}/supplies/maintain/upload`,formData,config).then( res => {
           if(res.data.code == '0'){
             setTimeout(() => {
            ...
            setTimeout(() => {
              this.$Message.error('该文件已存在,请重新导入!')
            },2000)
           setTimeout(() => {
             this.modal2 = false
             this.file = null
       ...
       */
       download() {
          setTimeout(() => {
              window.location.href='http://127.0.0.1:86/wh_model.xlsx'
       ...
       handleListApproveHistory(){
         ...
         changepage(index){
               ...
               this.loading = true
               this.$axios.get(`${this.URL_PREFIX}/supplies/maintain?page=${this.currentpage}&size=${this.pageSize}`).then( res => {
                   if(res.data.code == '0'){
                        this.data1 = res.data.result
                        this.loading = false
                    }
