'type':'固话(移动产权)',<template lang="html">
      <div class="abolish">
           <Row>
                      ...
                      <Form :label-width="120" :style="{marginLeft:'-55px',marginTop:'30px'}">
                          <FormItem label="表格导入:" :style="{width:'200px'}" id="myform"  v-if="file == null">
                            <Upload
                               ...
                               action="`${this.URL_PREFIX}/supplies/waste/upload?type=1`">
                       ...
                       <FormItem>
                          <Row>
                            ...
                            <Col span="4" class="btn">
                               ...
                               <Button type="ghost" icon="ios-cloud-upload-outline"  @click="Expor" :style="{backgroundColor:'#2786d3',color:'#edf6fb'}" >导出</Button>
                           </Col>
                          </Row>
                       </FormItem>
                    </Form>
...
export default {
     ...
     methods:{
       ...
       */
       ...
       getDataList() {
         this.$axios.get(`${this.URL_PREFIX}/supplies/waste?page=1&size=${this.pageSize}&type=1`).then( res => {
             if(res.data.code == '0'){
       ...
       */
       Search(){
         this.loading = true
         this.$axios.get(`${this.URL_PREFIX}/supplies/waste?page=1&size=${this.pageSize}&type=1&starttime=${this.starttime}&endtime=${this.endtime}&work=${this.station}&keyword=${this.keyword}`)
               .then(res => {
                 if(res.data.code == '0'){
       ...
       */
       Expor(){
             this.loading1 = true
             this.$axios.get(`${this.URL_PREFIX}/supplies/waste/export?type=1&starttime=${this.starttime}&endtime=${this.endtime}&work=${this.station}&keyword=${this.keyword}`).then( res => {
                 if(res.data.code == '0'){
                   setTimeout(() => {
                     this.loading1 = false
       ...
       upload(){
         ...
         this.$axios.post(`${this.URL_PREFIX}/supplies/waste/upload?type=1`,formData,config).then( res => {
           if(res.data.code == '0'){
             setTimeout(() => {
       ...
       handleListApproveHistory(){
         ...
         changepage(index){
               ...
               this.loading = true
               this.$axios.get(`${this.URL_PREFIX}/supplies/waste?page=${this.currentpage}&size=${this.pageSize}&type=1`).then( res => {
                   if(res.data.code == '0'){
                        this.data1 = res.data.result
                        this.loading = false
                    }
