<template lang="html">
     <div class="eight">
       <Row>
                   ...
                   <Row :style="{marginTop:'15px'}">
                       <FormItem label="图形展示:">
                            <RadioGroup v-model="category">
                               <Radio label="首次联系用户时长"></Radio>
                               <Radio label="首次联系用户时长占比"></Radio>
                               <Radio label="首次联系用户平均时长"></Radio>
                           </RadioGroup>
                       </FormItem>
                   </Row>
                </FormItem>
         ...
         <Row>
            <div class="linelist"  v-if="this.category=='首次联系用户时长'">
                <span>{{this.category}}</span>
                <contact-analyse :starttime="this.starttime" :endtime="this.endtime" :type="this.category"></contact-analyse>
            </div>
         </Row>
         <Row>
            <div class="linelist"  v-if="this.category=='首次联系用户时长占比'">
                <span>{{this.category}}</span>
                <contact-propotion :starttime="this.starttime" :endtime="this.endtime" :type="this.category"></contact-propotion>
            </div>
         </Row>
         <Row>
            <div class="linelist"  v-if="this.category=='首次联系用户平均时长'">
                <span>{{this.category}}</span>
                <contact-avetime :starttime="this.starttime" :endtime="this.endtime" :type="this.category"></contact-avetime>
            </div>
         </Row>
         <Row class="table">
           <Form ref="formCustom">
               <FormItem>
                   <Row>
                       ...
                       <Col span="6" class="btn">
                         ...
                         <Button type="ghost" icon="ios-cloud-upload-outline" :loading="loading1" :style="{backgroundColor:'#2786d3',color:'#edf6fb'}" @click="Expor">导出</Button>
                      </Col>
                   </Row>
               </FormItem>
           </Form>
...
<script>
import contactAnalyse from '../components/bar_analyse.vue'
import contactPropotion from '../components/bar_propotion.vue'
import contactAvetime from '../components/bar_ave.vue'
export default {
  data () {
      return {
          category:'首次联系用户时长',
          ...
          accounting:{},
          columns0: [
            { title: '首次联系用户时长',width:499,align: 'center' },
            { title: '首次联系用户时长占比', align: 'center' },
                              ...
                              on: {
                                click: () => {
                                 ...
                                 if(Number.isInteger(this.data3[params.index].value)){
                                   ...
                                   this.$axios.get(`${this.URL_PREFIX}/complain/emosFirst/info/export?type=${this.All}`)
                                              .then(res => {
                                                if(res.data.code == '0'){
                                                  setTimeout(() => {
  ...
  methods: {
    /*获取数据列表*/
    getDatalist(){
      this.$axios.get(`${this.URL_PREFIX}/complain/emosFirst/list?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`)
            .then(res => {
              if(res.data.code == '0'){
    ...
    */
    Expor() {
      ...
      }else if(this.endtimes ==''||this.endtimes == null){
        this.$Message.warning('请选择截止时间!')
      }else{
        this.loading1 = true
        this.$axios.get(`${this.URL_PREFIX}/complain/emosFirst/list/export?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`).then(res => {
          if(res.data.code == '0'){
            this.loading1 = false
            setTimeout(() => {
              this.$Message.success('导出成功')
