<template lang="html">
     <div class="twentyfour">
       <Row>
             ...
             <Form :style="{marginTop:'20px'}">
                <FormItem>
                   <Row>
                     ...
                     <Col span='7'>
                       <FormItem label="截止时间:">
                               <DatePicker type="datetime" placeholder="请选择" :value="endtimes" @on-change="handlechange"></DatePicker>
                       </FormItem>
                     </Col>
                     <!-- <Col span='10'>
                   ...
                   <Row :style="{marginTop:'15px'}">
                       <FormItem label="图形展示:">
                            <RadioGroup v-model="category">
                               <Radio label="投诉处理及时率"></Radio>
                               <Radio label="投诉平均处理时长"></Radio>
                               <Radio label="重复投诉率"></Radio>
                               <Radio label="万投比"></Radio>
                           </RadioGroup>
                       </FormItem>
                   </Row>
                </FormItem>
         ...
         <Row>
            <div class="linelist" v-if="this.category=='投诉处理及时率'">
                <span>{{this.category}}</span>
                <line-form :starttime="this.starttimes" :endtime="this.endtimes" :type="this.category"></line-form>
            </div>
         </Row>
         <Row>
            <div class="linelist" v-if="this.category=='投诉平均处理时长'">
                <span>{{this.category}}</span>
                <line-ave :starttime="this.starttimes" :endtime="this.endtimes" :type="this.category"></line-ave>
            </div>
         </Row>
         <Row>
            <div class="linelist" v-if="this.category=='重复投诉率'">
                <span>{{this.category}}</span>
                <line-repeat :starttime="this.starttimes" :endtime="this.endtimes" :type="this.category"></line-repeat>
            </div>
         </Row>
         <Row>
            <div class="linelist" v-if="this.category=='万投比'">
                <span>{{this.category}}</span>
                <line-investment :starttime="this.starttimes" :endtime="this.endtimes" :type="this.category"></line-investment>
            </div>
         </Row>
         <Row class="table">
           <Form ref="formCustom">
...
<script>
import lineForm from '../components/line1.vue'
import lineAve from '../components/line_ave.vue'
import lineRepeat from '../components/line_repeat.vue'
import lineInvestment from '../components/line_investment.vue'
export default {
  data () {
      return {
          category:'投诉处理及时率',
                          ...
                          h('Select', {
                                    ...
                                    on: {
                                        'on-change':(event) => {
                                            ...
                                            this.value = this.data3[params.index].value
                                        }
                                    },
                                },
                              ...
                              on: {
                                  click: () => {
                                    ...
                                    if(Number.isInteger(this.data3[params.index].value)){
                                      ...
                                      this.$axios.get(`${this.URL_PREFIX}/complain/twentyFour/info/export?type=${this.All}`)
                                                 .then(res => {
                                                   if(res.data.code == '0'){
                                                     setTimeout(() => {
  ...
  methods: {
    ...
    /*获取数据列表*/
    getDatalist(){
      this.$axios.get(`${this.URL_PREFIX}/complain/twentyFour/list?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`)
            .then(res => {
              if(res.data.code == '0'){
                //将获取的数据保存到data1空数组中
    ...
    */
    Expor() {
      if(this.order ==''||this.order == null){
        ...
        this.$axios.get(`${this.URL_PREFIX}/complain/twentyFour/list/export?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.Types}`)
        .then(res => {
          if(res.data.code == '0'){
            this.loading1 = false
            setTimeout(() => {
