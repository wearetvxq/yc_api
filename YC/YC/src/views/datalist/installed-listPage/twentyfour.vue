<template lang="html">
     <div class="twentyfour">
       <Row>
             ...
             <Form :style="{marginTop:'20px'}">
                <FormItem>
                   <Row>
                     ...
                     <!-- <Col span='10'>
                         <Button type="ghost" icon="ios-search-strong" :style="{backgroundColor:'#2786d3',color:'#edf6fb'}" @clcik="CheakOut">查看</Button>
                     </Col> -->
                   </Row>
         ...
         <Row>
           <div class="linelist1"  v-if="this.category=='平均装机时长'">
               <span>{{this.category}}</span>
...
<script>
import lineForm from '../components2/line.vue'
import lineDuraction from '../components2/line_duraction24.vue'
export default {
  data () {
      return {
          category:'装机及时率',
                              ...
                              on: {
                                click: () => {
                                  ...
                                  if(Number.isInteger(this.data3[params.index].value)){
                                    ...
                                    this.$axios.get(`${this.URL_PREFIX}/installed/twentyFour/info/export?type=${this.All}`)
                                               .then(res => {
                                                 if(res.data.code == '0'){
                                                   setTimeout(() => {
  ...
  // watch:{
  //      category: function() {
  //        this.$axios.get(`${this.URL_PREFIX}/complain/twentyFour/chart?starttime=${this.starttime}&endtime=${this.endtime}&type=${this.category}`)
  //        .then(res => {
  //           if(res.data.code == '0'){
  ...
  methods: {
    ...
    /*获取数据列表*/
    getDatalist(){
      this.$axios.get(`${this.URL_PREFIX}/installed/twentyFour/list?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`)
            .then(res => {
              if(res.data.code == '0'){
                //将获取的数据保存到data1空数组中
    ...
    */
    Expor() {
      if(this.order ==''||this.order == null){
        ...
        this.$axios.get(`${this.URL_PREFIX}/installed/twentyFour/list/export?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.Types}`)
        .then(res => {
          if(res.data.code == '0'){
            setTimeout(() => {
              this.loading1 = false
