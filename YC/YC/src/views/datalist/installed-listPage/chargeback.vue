       ...
       <Row>
         ...
         <Row>
            <div class="linelist">
                ...
                <line-charge :starttime="this.starttimes" :endtime="this.endtimes" :type="this.category" :types="this.model1"></line-charge>
            </div>
         </Row>
         <Row class="table">
           <Form ref="formCustom">
...
<script>
import lineCharge from '../components2/line_chargeback'
export default {
  data () {
      return {
          category:'退单率',
                              ...
                              on: {
                                  click: () => {
                                    ...
                                    if(Number.isInteger(this.data3[params.index].value)){
                                      ...
                                      this.$axios.get(`${this.URL_PREFIX}/installed/retreat/info/export?type=${this.All}`)
                                                 .then(res => {
                                                   if(res.data.code == '0'){
                                                     setTimeout(() => {
  ...
  methods: {
    /*获取数据列表*/
    getDatalist(){
      this.$axios.get(`${this.URL_PREFIX}/installed/retreat/list?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`)
            .then(res => {
              if(res.data.code == '0'){
                //将获取的数据保存到data1空数组中
    ...
    */
    Expor() {
        this.loading1 = true
        this.$axios.get(`${this.URL_PREFIX}/installed/retreat/list/export?starttime=${this.starttimes}&endtime=${this.endtimes}&type=${this.model1}`).then(res => {
          if(res.data.code == '0'){
            this.loading1 = false
            setTimeout(() => {
                this.$Message.success('导出成功')
