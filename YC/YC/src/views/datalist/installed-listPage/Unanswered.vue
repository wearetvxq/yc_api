     ...
     <div class="Tops" :style="{marginTop:'-50px'}">
         <Row class="table">
           <Form ref="formCustom">
               <FormItem>
                   <Row>
                        <h3 :style="{marginBottom:'10px'}">超时未回复</h3>
                       <Col span="6">
                           <FormItem label="起始时间:">
                               <Col span="13">
...
export default {
                  ...
                  render: (h, params) => {
                      return h('div', [
                          h('Button', {
                              ...
                              on: {
                                click: () => {
                                  this.$axios.get(`${this.URL_PREFIX}/installed/out/info/export?type=${params.row.ID}`)
                                             .then(res => {
                                               if(res.data.code == '0'){
                                                 setTimeout(() => {
  ...
  methods: {
    /*获取数据列表*/
    getDatalist(){
      this.$axios.get(`${this.URL_PREFIX}/installed/out/list?starttime=${this.starttime}&endtime=${this.endtime}&type=${this.model1}`)
            .then(res => {
              if(res.data.code == '0'){
                //将获取的数据保存到data1空数组中
    ...
    */
    Expor() {
        this.loading1 = true
        this.$axios.get(`${this.URL_PREFIX}/installed/out/list/export?starttime=${this.starttime}&endtime=${this.endtime}&type=${this.model1}`).then(res => {
          if(res.data.code == '0'){
            this.loading1 = false
            setTimeout(() => {
                this.$Message.success('导出成功')
