...
def supplies_maintain_export_get(starttime=None, endtime=None, work=None, type=None, keyword=None):  # noqa: E501
    ...
    try:
        ...
        msg = {
            "msg": "msg",
            "code": 0,
            "url": "down.caicool.cc:89/{}".format(name),
        }
    except:
        msg = {
    ...
    try:
        ...
        for i in range(1, len(num)):
            try:
                ...
                # 加一种 信息重复的情况
                if YCmaintain.select().where(YCmaintain.order == order).count() == 1:
...
def supplies_statistics_install_export_get(page, size, type, starttime=None, endtime=None, work=None, model=None,
    ...
    return ({'code': 0, 'msg': 'success', 'url': 'down.caicool.cc:89/{}.xls'.format(xlsname)})
        ...
        for i in range(1, len(num)):
            try:
                ...
                try:
                    ...
                    posttime = item[9]
                #加一种 信息重复的情况
                if YCuse.select().where(YCuse.order==order).count()==1:
                    msg = {'code': 10003, 'msg': i}
                    return msg
