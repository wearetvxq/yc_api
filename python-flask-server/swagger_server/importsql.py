            ...
            for x in range(1, sheet.nrows):
                    try:
                        ...
                        if msg[9] != 'order_status_6':
                            ...
                            if YCinstalled.select().where(YCinstalled.orderid==msg[4]).count()==0:
                                YCinstalled.insert(type=get_type(msg[3]), area=result[0], status='order_status_5',
                                                   timeout=result[2],
                                                   ...
                                                   orders=utc_str_to_timestamp(msg[13]), receipt=msg[14],
                                                   confirm=endtime,
            ...
            if name=='EMOS':
                ...
                for x in range(1, sheet.nrows):
                    ...
                    if msg[8] != '待回访' and msg[18]!='' :
                        ...
                        if YCcomplaints.select().where(YCcomplaints.serial==msg[1]).count()==0:
                           YCcomplaints.insert(area=result[0],platform=2,status=result[1],type='服务质量',timeout=result[2],starttime=str(utc_str_to_timestamp(str(xlrd.xldate_as_datetime(msg[16],0)))),endtime=str(utc_str_to_timestamp(str(xlrd.xldate_as_datetime(msg[18],0)))),pid=pid,usetime='%.2f' % result[-2],contacts=msg[4],phone=msg[12],fristtime=str(utc_str_to_timestamp(str(xlrd.xldate_as_datetime(msg[66],0)))),serial=msg[1],t2endtime=str(utc_str_to_timestamp(str(xlrd.xldate_as_datetime(msg[19],0)))),manyi=getmanyi(msg[15]),tsaddress=result[-1],network=msg[-1],posttime=int(time.time())).execute()
        ...
        for name in workbook_r.sheet_names():
            if name=='一键报障':
                ...
                for x in range(1, sheet.nrows):
                    ...
                    if YCcomplaints.select().where(YCcomplaints.serial == msg[1]).count() == 0:
                       YCcomplaints.insert(area=result[0],platform=1,type='服务质量',status=result[1],timeout=result[2],network=msg[9],starttime=utc_str_to_timestamp(str(msg[22])),endtime=msg[25],pid=pid,posttime=int(time.time()),usetime=result[-2],contacts=msg[26],phone=msg[15],fristtime=msg[24],serial=msg[1],manyi=2,tsaddress=result[-1]).execute()
        for name in workbook_r.sheet_names():
            if name=='CRM':
                ...
                for x in range(1, sheet.nrows):
                    ...
                    if msg[6]!='':
                        ...
                        endtime=starttime
                    if msg[4].find('服务质量') != -1:
                        msg[4] = '服务质量'
                    if msg[4].find('服务质量') == -1:
                        msg[4] = '基础业务'
                    if YCcomplaints.select().where(YCcomplaints.serial == msg[2]).count() == 0:
                       YCcomplaints.insert(area=result[0],serial=msg[2],platform=3,type=msg[4],network='',status=result[1],timeout=result[2],starttime=starttime,endtime=endtime,pid=pid,manyi=2,posttime=int(time.time()),usetime=result[-2],tsaddress=result[-1]).execute()
#import_sql('/root/桌面/2月份投诉原始数据.xlsx','1',9)
#import_sql('/root/桌面/装机.xlsx','2',1)

if __name__ == '__main__':

    import_sql('/var/www/downfile/装机信息590.xlsx','2',2)
